...
1. **Wait for the image build to finish** — LSF job **`295400367`** on the `fakeroot` queue. Build history: `295393855` failed on a fakeroot permission-traversal issue (fixed by staging under `/tmp`); `295400359` completed the full conda install but failed the `%test` because the `pbskera` package installs its binary as `skera` (fixed by adding a `pbskera -> skera` symlink in `%post` and correcting the test).
